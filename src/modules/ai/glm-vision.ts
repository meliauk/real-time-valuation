 ...
 * 只识别4个字段：基金名称、基金代码、持有金额、持有收益。
 *   - 持有金额/持有收益用于持仓编辑（填入持仓记录），收益率等不识别（系统自算）。
 *   - 支付宝列表每行从左到右 4 个数字：①持有金额 ②昨日收益 ③持有收益 ④持有收益率(%)，
 *     要取 ①+③。模型易把 ②昨日收益 误当 ③持有收益，故 prompt 用列位置硬锚定（第3个金额
 *     = holdingProfit，第2个=昨日收益不取），仅靠字段名描述模型仍会蒙错——见验证页验证结论。
 */
