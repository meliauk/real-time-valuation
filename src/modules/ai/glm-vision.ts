...
- 如果图中某只基金的某个数字看不清、被遮挡或不存在，则对应字段必须省略（不要返回该字段），绝对不能用 0、null 或估算值填充。
- 累计收益若为 0，只有当图中明确印着"0.00"或"0"时才能填 0；看不清就省略该字段，不要填 0。

【关键·必须完整扫描整张图，尤其是右侧】
- 这张截图较宽，信息从左到右分布在多列：左侧通常是基金名称/代码，右侧常常是持有金额、累计收益等关键数值。
- 你容易只看左半部分而漏掉右半部分。请强制自己从图像最左扫到最右、从顶部扫到底部，确认每一行的每一列都读到，特别是图像右 1/3 区域的数字。
- 逐行处理：对图中每一只基金，必须同时读出该行左侧（名称/代码）和右侧（持有金额/累计收益）的所有信息，缺一不可。右侧的「累计收益」「持有金额」列往往就是你要识别的数值，切勿跳过。
- 识别完成后自查：你返回的每只基金是否都有 holdingAmount 和 accumulatedProfit？如果大量基金这两个字段缺失，很可能是你漏看了右侧列，请重新扫描右侧再输出。

识别指引：
- 每只基金在截图中通常有「持有金额」和「累计收益」两个数字，必须分别读出。
- 「累计收益」可能是独立一行（如"累计收益 234.56"），也可能是"累计收益 + 百分比 + 金额"，请提取其中的金额数值。
- 持有金额常见叫法："持有金额"/"持仓金额"/"当前持有"/"总资产"，统一映射为 holdingAmount。
- 累计收益常见叫法："累计收益"/"累计盈亏"/"累计回报金额"，统一映射为 accumulatedProfit（百分比形式的"累计收益率"不是金额，不填）。

注意：
1. 只识别上述4个字段，不要识别收益率/涨跌幅/当日收益等（系统自算）
2. accumulatedProfit 必须是金额数值（元），不是百分比
3. 只返回JSON数组，不要返回任何其他文字说明
4. 看不清/不存在的字段直接省略，不要填 0 或估算
5. 累计收益亏损必须为负数（如 -123.45），看不清就省略该字段，不要填 0
